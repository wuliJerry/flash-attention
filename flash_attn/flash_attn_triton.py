...
def _fwd_kernel(
    ...
    # NOTE: Line 4 of the Algorithm1, applied m(mask) when reading the q, k, v
    if EVEN_M & EVEN_N:
        if EVEN_HEADDIM:
            q = tl.load(q_ptrs)
        else:
    ...
    # NOTE: Line 4 ,output tensor O is stored depending on the conditions EVEN_M and EVEN_HEADDIM.
    if EVEN_M:
        if EVEN_HEADDIM:
